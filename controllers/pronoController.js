...
exports.newProno = async (req, res) =>{
    try {
        ...
        if(result3.rows.length === 0){
            ...
            const result = await pool.query(query, [match.match_id, match.player1_id, match.player2_id, null, match.tournament, match.start_time, match.status ])
        }

        const query2 = 'INSERT INTO "Pronostics"(user_id, match_id, prono, result) VALUES($1, $2, $3, $4) RETURNING *'
        const result2 = await pool.query(query2, [req.user.id, match.match_id, prono, null])
